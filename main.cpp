...
int APIENTRY WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    if (!doneLogin) {
        ...
        std::string consoleTitle = (std::string)skCrypt("Luna Spoofer");
        SetConsoleTitleA(consoleTitle.c_str());
        ShowWindow(GetConsoleWindow(), SW_SHOW);
        std::cout << termcolor::bright_yellow << skCrypt("\n [!] Checking if your Winver is Supported... ");
        Sleep(3000);
        ...
        {
            system("cls");
            std::cout << termcolor::bright_yellow << skCrypt ("\n [!] Your Winver is supported! Your Windows version is\n ") << osVersionInfo.dwMajorVersion << "." << osVersionInfo.dwMinorVersion << std::endl;
            Sleep(3000);
            system("cls"); 
        }
        else 
        {
            system("cls");
            std::cout << termcolor::bright_yellow << skCrypt ("\n Error getting Windows version.") << std::endl;
            Sleep(3000);
            system("cls");
        }
        std::cout << termcolor::bright_yellow << skCrypt("\n [!] Enter your license: ");
    ...
    if (doneLogin) {
        ...
        while (msg.message != WM_QUIT)
        {
            ...
            ImGui::NewFrame();
            {
                ...
                ImGui::Begin("Luna Spoofer", &loader_active, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoBringToFrontOnFocus);
                {
                    ImGui::StyleColorsClassic(); // Purple theme
                    ImGui::TextColored(ImVec4(r - 0.15f, g - 0.35f, b, 1.0f), ("HWID SPOOFER"));

                    ImGui::Spacing();
                    if (ImGui::BeginTabBar("#tabs")) {
                        if (ImGui::BeginTabItem("Main")) 
                        {
                            ImGui::Text("This Spoofer Null's your Disk.");
                            ImGui::Text("It only Supports the Anti Cheat BattleEye!");
                            ImGui::Spacing();
                            if (ImGui::Button("Spoof"));
                            {
                                MessageBox(NULL, "Loading Kernel Modules", "Information", MB_OK | MB_ICONINFORMATION);
                                HANDLE iqvw64e_device_handle = intel_driver::Load();

                                if (!iqvw64e_device_handle || iqvw64e_device_handle == INVALID_HANDLE_VALUE)
                                {
                                    MessageBox(NULL, "Error code iq00001, please disable any anti-cheat such as valorant or faceit and try again", "Error", MB_OK | MB_ICONINFORMATION);
                                ...
                                intel_driver::Unload(iqvw64e_device_handle);
                                MessageBox(NULL, "Spoofed!", "Information", MB_OK | MB_ICONINFORMATION);
                            }
                            ImGui::Spacing();
                            ImGui::EndTabItem();
                        }
